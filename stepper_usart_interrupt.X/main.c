...
char a;
//unsigned char dir;
unsigned char Str[4]; // receive freq
uint16_t hex;
int int_freq=0, int_dir = 0; // freq value
int accel=1;
unsigned char dir;

// FLASH ADRESS TO STORE LAST FREQUENCY
...
void set_freq()
{
        ...
        if( targetFreq > lastFrequency) // accel code
        {
           ...
           // hex = freqCalc(targetFreq);
        }
        else if( targetFreq < lastFrequency) // deccel code
        {
            int deccelStep = round((lastFrequency - targetFreq)/50);
            for(int j=0; j<50;j++)
            {
