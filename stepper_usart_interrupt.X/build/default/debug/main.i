...
int take_freq()
{
    UART_Send("Enter frequency and press enter");
    int i=0;
    char input_frequency[] = "\0";
    char tp = RCREG1;
    do{
        input_frequency[i] = RCREG1;
        i+=1;
    }while((tp!='\n') && (tp!='\r'));
    input_frequency[i+1] = "\0";
    int inp_freq_num = atoi(input_frequency);
    UART_Send("You entered the following frequency\n");
    UART_Send(input_frequency);
    return inp_freq_num;
}
